...
BEGIN
    ...
    IF INSERTING THEN 
        v_operation := 'INSERT';
        v_before := NULL;
        v_after := 'KONTAKT_ID: ' || :NEW.KONTAKT_ID || ', EMAIL: ' || :NEW.EMAIL || ', TELEFON: ' || :NEW.TELEFON || ', OSOBA_ID: ' || :NEW.OSOBA_ID;
    ELSIF UPDATING THEN 
        v_operation := 'UPDATE';
        v_before := 'KONTAKT_ID: ' || :OLD.KONTAKT_ID || ', EMAIL: ' || :OLD.EMAIL || ', TELEFON: ' || :OLD.TELEFON || ', OSOBA_ID: ' || :OLD.OSOBA_ID;
        v_after := 'KONTAKT_ID: ' || :NEW.KONTAKT_ID || ', EMAIL: ' || :NEW.EMAIL || ', TELEFON: ' || :NEW.TELEFON || ', OSOBA_ID: ' || :NEW.OSOBA_ID;
    ELSIF DELETING THEN 
        v_operation := 'DELETE';
        v_before := 'KONTAKT_ID: ' || :OLD.KONTAKT_ID || ', EMAIL: ' || :OLD.EMAIL || ', TELEFON: ' || :OLD.TELEFON || ', OSOBA_ID: ' || :OLD.OSOBA_ID;
